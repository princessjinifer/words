...
void extract () // extracts the high number from the file
{
	...
	if (highnum.is_open()) // if its open then:
	{
		...
		{
			getline (highnum,hnum); // get the file contents
		}
		highnum.close(); // close teh file
	}
	// this next section most likely can be deleted, have not had a use for it in a while...
/*	ofstream number ("number", ios::trunc); // open the number file
	if (number.is_open()) // if its open
	{
		number.close(); // close teh file
	}
	ifstream highnum ("number"); // gotta get the high number
	if (highnum.is_open()) // if its open:
	{
		while ( highnum.good() ) // :/
		{
			getline (highnum,hnum); // get its thing
		}
		highnum.close(); // close it
	}*/
//	else cout << "Unable to open file"; // sowwie :(
...
//	cout << rando << endl; // used for developer purposes only
	...
	while (true) // if umm, nawt exactly sure about this one either
	{
